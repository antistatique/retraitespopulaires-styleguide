...
class PLPRatesRepository {
   ...
   * Tableau Homme.
   *
   * Des taux de conversion  des PLP à 3% (valable dès le 1.01.2012)
   *
   * @var array
   */
  const PLP_CONVERSION_RATES_MEN = [
    60 => [
      0 => 5.958,
      40 => 5.298,
      60 => 5.021,
   ...
   * @var array
   */
  const PLP_CONVERSION_RATES_WOMEN = [
    57 => [
      0 => 4.700,
      40 => 4.640,
      60 => 4.611,
  ...
  public function __construct(PLPInterestRate $plp_interest_rate, PLPConversionRate $plp_conversion_rate) {
    $this->plpInterestRate = $plp_interest_rate;
  ...
  public function getRate($year) {
    ...
    $rate = $this->plpInterestRate->getRate($year);
  ...
  public function getConversionRate($gender, $age, $percent) {
    ...
    $rate = $this->plpConversionRate->getRate($gender, $age, $percent);
