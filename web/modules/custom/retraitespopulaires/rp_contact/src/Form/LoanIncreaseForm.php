...
class LoanIncreaseForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $params = NULL) {
    ...
    $form['loan_increase']['amount'] = [
      '#title'       => $this->t('Pour un montant *'),
    ...
    $form['loan_increase']['rate'] = [
      '#title'       => $this->t('Réserver le taux *'),
      '#type'        => 'select',
      '#required'    => FALSE,
      '#options'     => $options,
      '#prefix'      => '<div class="form-group ' . $error_class . '">',
    ...
    $form['files'] = [
      '#type'       => 'fieldset',
      '#attributes' => ['class' => ['fieldset-no-legend ']],
      '#title'      => $this->t('Vos documents'),
      '#prefix'     => '<h3 class="card-title">' . $this->t('Vos documents') . '</h3>',
    ];

    $form['files']['file_estimate'] = [
      '#type'       => 'file',
      '#attributes' => ['title' => $this->t('Choisissez votre fichier'), 'label' => $this->t('Devis (en cas de travaux)')],
    ...
    $form['files']['file_tax'] = [
      '#type'       => 'file',
      '#attributes' => ['title' => $this->t('Choisissez votre fichier'), 'label' => $this->t('Dernière déclaration fiscale')],
      '#prefix'     => '<div class="form-group form-file">',
      '#suffix'     => '</div>',
    ];

    $form['files']['file_other'] = [
      '#type'       => 'file',
      '#attributes' => ['title' => $this->t('Choisissez votre fichier'), 'label' => $this->t('Autre')],
      '#prefix'     => '<div class="form-group form-file">',
      '#suffix'     => '</div>',
    ];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!$form_state->getValue('purpose') || empty($form_state->getValue('purpose'))) {
      $errors['purpose'] = $this->t('Votre but est obligatoire.');
    }
