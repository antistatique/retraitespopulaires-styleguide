...
const buildNoClean = gulp.series(
  vendors,
  icons,
  single,
  styles,
...
gulp.task("build-no-clean", buildNoClean);

/**
 * Default task
 */
const defaultFunc = (done, isServe) =>
  loader
    .import(conditionalStyleguide)
    .then((m) => {
 ...
 * Metalsmith task
 */
const metalsmithTask = gulp.task(
  "metalsmith",
  yargs.argv.production ? inprod : require("./tasks/metalsmith").metalsmith
);

/**
 * Watch task
 */
gulp.task("watch-docker", function (done) {
  watch(
    "./assets/**/*",
    { interval: 2000, usePolling: true },
    gulp.series("build-no-clean")
  );
  done();
});
